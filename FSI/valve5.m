function valve5(BCIndex,t,cnt,q,h,hs )
%VALVE5 计算阀门前后两个节点的压力和流量,论文中的方法,但是有正负变化，利用当前值
%   计算两个管道之间的阀门的情况
%这种情况可以视为管道面积的突然变化
% 输入变量
...
%             %2017-1-11增加判断压力值的正负
            ...
            elseif qp(pipe1,nn(pipe1))<0||isreal(qp(pipe1,nn(pipe1)))
                a=ca(pipe1)*ca(pipe2)/2/g/A_lv; %修改2017-1-11
                b=-(ca(pipe1)+ca(pipe2));
                c=ca(pipe1)*cn_val+ca(pipe2)*cp_val;
                qp(pipe1,nn(pipe1))=-b/(2*a)-sqrt((b/(2*a))^2-c/a);
